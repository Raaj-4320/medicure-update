...
import { logOrderTrace } from '../utils/orderTraceLogger';
import type {
  ComplianceDocument,
  DeliveryAssignment,
  MedicineMaster,
...
async function createOrder(payload: Dictionary): Promise<Order> {
  const traceId = asString(payload.traceId, makeTraceId('ord'));
  logOrderTrace('CREATE_ORDER_START', {
    traceId,
  ...
  logOrderTrace('CREATE_ORDER_PHARMACY_CHECK', {
    traceId,
    pharmacyId,
    found: Boolean(pharmacy),
    status: asString(pharmacy?.status || pharmacy?.verificationStatus, 'missing'),
    complete: pharmacy ? isPharmacyProfileComplete(pharmacy as Pharmacy & { verificationDetails?: Dictionary }) : false,
  });
  if (!pharmacy || !isPharmacyOperational(pharmacy as Pharmacy & { verificationDetails?: Dictionary })) {
    logOrderTrace('CREATE_ORDER_BLOCKED', {
      traceId,
      reason: 'pharmacy_not_operational',
      pharmacyId,
    });
    throw new Error('Order blocked: pharmacy is not operational.');
  }
  const inventory = await getInventory({ pharmacyId });
  logOrderTrace('CREATE_ORDER_INVENTORY_CHECK', {
    traceId,
    pharmacyId,
    inventoryCount: inventory.length,
  });
  ...
  logOrderTrace('CREATE_ORDER_SUCCESS', {
    traceId,
    orderId: id,
    customerId: asString(orderPayload.customerId),
    pharmacyId: asString(orderPayload.pharmacyId),
    totalAmount: asNumber(orderPayload.totalAmount),
  });
...
async function getOrders(filters: FilterOptions = {}): Promise<Order[]> {
  ...
  if (sortedOrders.length > 0) {
    ...
    logOrderTrace('GET_ORDERS_DB_RESULT', {
      filters,
  ...
  logOrderTrace('GET_ORDERS_DB_RESULT', {
    filters,
    count: sortedOrders.length,
    source: fallback.length > 0 ? 'cache' : 'db-empty',
    cacheCount: fallback.length,
  });
  ...
  try {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snap) => {
        ...
        logOrderTrace('SUBSCRIBE_ORDERS_SNAPSHOT', {
          filters,
          count: orders.length,
          source: 'db',
        });
      ...
      () => {
        const cached = readCache<Order>('orders', filters);
        logOrderTrace('SUBSCRIBE_ORDERS_ERROR_FALLBACK', {
          filters,
    ...
    const cached = readCache<Order>('orders', filters);
    logOrderTrace('SUBSCRIBE_ORDERS_THROW_FALLBACK', {
      filters,
      cacheCount: cached.length,
      error: String(error),
    });
...
function getFallbackByName(name: string): unknown {
  if (name.startsWith('subscribeTo')) {
    return () => undefined;
  }
