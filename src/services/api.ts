  ...
  const visibilityChecks = await Promise.all(
    verifiedPharmacies.map(async (pharmacy) => {
      ...
      return {
        ...
        hasCoreProfile: Boolean(pharmacy.id) && Boolean(pharmacy.name),
      };
    }),
  );
  let visiblePharmacies = visibilityChecks
    .filter((entry) => entry.hasCoreProfile && entry.inventoryCount > 0)
    .map((entry) => entry.pharmacy);
  if (visiblePharmacies.length === 0) {
    visiblePharmacies = visibilityChecks.filter((entry) => entry.hasCoreProfile).map((entry) => entry.pharmacy);
  }
  logFlow('GET_PHARMACIES_FOR_CUSTOMER', {
    expected: ['verified pharmacy visible for customer demo flow'],
    ...
    suggestion: 'Ensure at least one verified pharmacy has a visible profile.',
  });
...
async function updateMedicine(id: string, patch: Dictionary): Promise<boolean> {
  ...
  const [masterSnap, medicinesSnap] = await Promise.all([
    getDoc(doc(db, 'medicine_master', id)),
    getDoc(doc(db, 'medicines', id)),
  ]);
  const updates: Promise<boolean>[] = [];
  if (masterSnap.exists()) {
    updates.push(patchDoc('medicine_master', id, normalizedPatch));
  }
  if (medicinesSnap.exists()) {
    updates.push(patchDoc('medicines', id, normalizedPatch));
  }
  if (updates.length === 0) {
    return false;
  }
...
async function createOrder(payload: Dictionary): Promise<Order> {
  ...
  if (!isPharmacyOperational(pharmacy as Pharmacy & { verificationDetails?: Dictionary })) {
    appLogger.log({
      category: 'SYSTEM_WARNING',
      event: 'api_create_order_non_operational_pharmacy',
      status: 'warning',
      page: 'api',
      scope: 'createOrder',
      message: 'Pharmacy profile is incomplete/unverified, but order flow remains enabled for demo.',
      ids: { pharmacyId },
    });
  }
  ...
  if (inventory.length === 0) {
    appLogger.log({
      category: 'SYSTEM_WARNING',
      event: 'api_create_order_inventory_empty',
      status: 'warning',
      page: 'api',
      scope: 'createOrder',
      message: 'Pharmacy inventory query returned empty during order create.',
      ids: { pharmacyId },
    });
  }
