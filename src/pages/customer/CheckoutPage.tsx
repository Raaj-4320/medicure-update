...
export default function CheckoutPage() {
  ...
  const hasValidSelectedAddress = availableAddresses.some((addr: any) => String(addr.id) === String(selectedAddress || ''));
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const selectedAddressData =
        normalizedAddresses.find((addr: any) => addr.id === String(selectedAddress || '')) || null;
      if (!selectedAddressData) {
        setError('Please select a delivery address before placing the order.');
        return;
      }
      appLogger.log({
        category: 'ORDER_FLOW',
              ...
              {step >= 2 && (
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl border border-emerald-200 bg-emerald-50/40">
                    <p className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-2">Delivery Address For This Order</p>
                    {hasValidSelectedAddress ? (
                      <>
                        {availableAddresses
                          .filter((addr: any) => String(addr.id) === String(selectedAddress))
                          .map((addr: any) => (
                            <div key={addr.id} className="text-sm text-slate-700">
                              <p className="font-bold text-slate-900">{addr.type}</p>
                        ...
                        {availableAddresses.length > 1 && (
                          <div className="mt-3">
                            <label className="block text-xs font-semibold text-slate-600 mb-1">Change address</label>
                            <select
                  ...
                  {step === 2 && (
                    <button 
                      ...
                      disabled={loading || !hasValidSelectedAddress}
