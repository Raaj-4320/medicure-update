...
const ORDER_SETTLED_STATUSES = new Set(['delivered', 'picked_up']);
  ...
  const load = async () => {
    ...
    try {
      ...
      const paymentByOrderId = payments.reduce<Record<string, any>>((acc, payment: any) => {
        ...
        if (!FINANCIAL_SUCCESS_STATUSES.has(status)) return acc;
        acc[orderId] = payment;
        return acc;
      }, {});
      ...
      const financiallyValidOrders = sellerOrders.filter((order: any) => {
        ...
        const hasSuccessfulPayment = FINANCIAL_SUCCESS_STATUSES.has(orderPaymentStatus) || Boolean(paymentByOrderId[String(order.id)]);
        return ORDER_SETTLED_STATUSES.has(status) && hasSuccessfulPayment;
      });
      ...
      const returnDeduction = sellerReturns.reduce((sum: number, entry: any) => {
        ...
        const itemLevel = Array.isArray(entry.items)
          ? entry.items.reduce((itemSum: number, item: any) => itemSum + (Number(item.quantity || 0) * Number(item.price || 0)), 0)
          : 0;
        const fallback = Number(linkedOrder?.totalAmount || 0);
      ...
      const pendingAmount = normalizedPayouts
        .filter((payout) => ['pending', 'processing'].includes(String(payout.status).toLowerCase()))
        .reduce((sum, payout) => sum + Number(payout.netAmount || 0), 0);
      ...
      sellerReturns.forEach((entry: any) => {
        ...
        const itemLevel = Array.isArray(entry.items)
          ...
          : 0;
        const fallback = Number(linkedOrder?.totalAmount || 0);
        const deduction = itemLevel > 0 ? itemLevel : fallback;
