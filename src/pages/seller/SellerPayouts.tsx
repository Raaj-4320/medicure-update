...
const FINANCIAL_FAILURE_STATUSES = new Set(['failed', 'cancelled', 'canceled', 'refunded']);
  ...
  const load = async () => {
    ...
    try {
      ...
      const paymentByOrderId = payments.reduce<Record<string, any>>((acc, payment: any) => {
        ...
        if (FINANCIAL_FAILURE_STATUSES.has(status)) return acc;
        acc[orderId] = payment;
        return acc;
      }, {});
      ...
      const financiallyValidOrders = sellerOrders.filter((order: any) => {
        ...
        const hasSuccessfulPayment = FINANCIAL_SUCCESS_STATUSES.has(orderPaymentStatus) || FINANCIAL_SUCCESS_STATUSES.has(paymentRecordStatus);
        const isCashOnDelivery = String(order.paymentMethod || paymentRecord?.paymentMethod || '').toLowerCase() === 'cash';
        const hasNonFailedPayment = !FINANCIAL_FAILURE_STATUSES.has(orderPaymentStatus) && !FINANCIAL_FAILURE_STATUSES.has(paymentRecordStatus);
        const hasLegacySettledNoPaymentSignal = !orderPaymentStatus && !paymentRecordStatus;
        return ORDER_SETTLED_STATUSES.has(status) && (hasSuccessfulPayment || (isCashOnDelivery && hasNonFailedPayment) || hasLegacySettledNoPaymentSignal);
      });
      ...
      const sellerReturns = returns.filter((entry: any) => {
        ...
        if (pharmacyIds.has(entry.pharmacyId)) return true;
        const linkedOrder = orderById[String(entry.orderId || '')];
        return Boolean(linkedOrder && (asSellerMatch(linkedOrder.sellerId, profile.uid) || (linkedOrder.items || []).some((item: any) => asSellerMatch(item?.sellerId, profile.uid))));
      });
