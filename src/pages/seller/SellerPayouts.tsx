  ...
  const load = async () => {
    ...
    try {
      ...
      if (pharmacyIds.size === 0) {
        setPayouts([]);
        setWallet({ availableBalance: 0, totalEarningsNet: 0, pendingSettlement: 0, commissionPaid: 0, gstDeducted: 0 });
        setChartData([]);
        return;
      }
      ...
      const financiallyValidOrders = sellerOrders.filter((order: any) => {
        ...
        return ORDER_SETTLED_STATUSES.has(status) && (hasSuccessfulPayment || (isCashOnDelivery && hasNonFailedPayment));
      });

      const refundStatuses = new Set(['approved', 'processed', 'completed', 'refunded']);
      const sellerReturns = returns.filter((entry: any) => pharmacyIds.has(entry.pharmacyId) && refundStatuses.has(String(entry.status || '').toLowerCase()));
