...
export default function SellerOrders() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const myPharmacy = pharmacies[0];
      if (myPharmacy?.id) {
        const subscriptionResult = api.subscribeToOrders({ pharmacyId: myPharmacy.id }, (liveOrders: any[]) => {
          setOrders(liveOrders);
        });
        unsubscribe = typeof subscriptionResult === 'function' ? subscriptionResult : null;
      }
    };
    init();
    return () => {
