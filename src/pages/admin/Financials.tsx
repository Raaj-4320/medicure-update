              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {sellerIncomeRows.length === 0 ? (
                    ...
                    <div key={row.pharmacyId} className="border border-slate-100 rounded-lg px-3 py-3 flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold text-slate-900">{row.sellerName}</p>
                      <p className="text-lg font-bold text-emerald-600 whitespace-nowrap">₹{row.totalIncome.toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
