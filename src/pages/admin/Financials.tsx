...
const Financials: React.FC = () => {
  ...
  const [customerSpendRows, setCustomerSpendRows] = useState<Array<{ customerId: string; customerName: string; totalSpend: number; orderCount: number }>>([]);
  const [sellerIncomeRows, setSellerIncomeRows] = useState<Array<{ pharmacyId: string; sellerName: string; totalIncome: number; paidOut: number; pendingPayout: number }>>([]);
  ...
  const loadFinancials = async () => {
    try {
      setLoading(true);
      const [payouts, orders, payments, returns, users, pharmacies] = await Promise.all([
        api.getPayouts(),
        api.getOrders(),
        api.getPayments(),
        api.getReturns(),
      ...
      const usersById = users.reduce<Record<string, any>>((acc, user: any) => {
        acc[String(user.uid || user.id)] = user;
        return acc;
      }, {});
      const pharmaciesById = pharmacies.reduce<Record<string, any>>((acc, pharmacy: any) => {
        acc[String(pharmacy.id)] = pharmacy;
        return acc;
      }, {});

      const customerSpendMap = orders.reduce<Record<string, { totalSpend: number; orderCount: number }>>((acc, order: any) => {
        const key = String(order.customerId || 'unknown');
        if (!acc[key]) acc[key] = { totalSpend: 0, orderCount: 0 };
        acc[key].totalSpend += Number(order.totalAmount || 0);
        acc[key].orderCount += 1;
        return acc;
      }, {});
      setCustomerSpendRows(
        Object.entries(customerSpendMap)
          .map(([customerId, data]) => ({
            customerId,
            customerName: usersById[customerId]?.displayName || usersById[customerId]?.email || customerId,
      ...
      const sellerIncomeMap = orders.reduce<Record<string, { totalIncome: number }>>((acc, order: any) => {
        const key = String(order.pharmacyId || 'unknown');
        if (!acc[key]) acc[key] = { totalIncome: 0 };
        acc[key].totalIncome += Number(order.totalAmount || 0);
        return acc;
      }, {});
      const payoutMap = normalizedPayouts.reduce<Record<string, { paidOut: number; pendingPayout: number }>>((acc, payout: any) => {
        const key = String(payout.pharmacyId || 'unknown');
        if (!acc[key]) acc[key] = { paidOut: 0, pendingPayout: 0 };
        if (String(payout.status || '').toLowerCase() === 'paid') acc[key].paidOut += Number(payout.sellerPayout || 0);
        else acc[key].pendingPayout += Number(payout.sellerPayout || 0);
        return acc;
      }, {});
      setSellerIncomeRows(
        Object.entries(sellerIncomeMap)
          .map(([pharmacyId, data]) => ({
            pharmacyId,
            sellerName: pharmaciesById[pharmacyId]?.name || pharmaciesById[pharmacyId]?.email || pharmacyId,
            totalIncome: Number(data.totalIncome.toFixed(2)),
            paidOut: Number((payoutMap[pharmacyId]?.paidOut || 0).toFixed(2)),
            pendingPayout: Number((payoutMap[pharmacyId]?.pendingPayout || 0).toFixed(2)),
          }))
        ...
        {activeTab === 'overview' && (
          <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-900 mb-4">Customer Spend</h3>
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {customerSpendRows.length === 0 ? (
                    <p className="text-sm text-slate-500">No customer spend data available yet.</p>
                  ) : customerSpendRows.map((row) => (
                    <div key={row.customerId} className="flex items-center justify-between border border-slate-100 rounded-lg px-3 py-2">
                      <div>
                      ...
                      <p className="text-sm font-bold text-slate-900">₹{row.totalSpend.toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-900 mb-4">Seller Income</h3>
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {sellerIncomeRows.length === 0 ? (
                    <p className="text-sm text-slate-500">No seller income data available yet.</p>
                  ) : sellerIncomeRows.map((row) => (
                    <div key={row.pharmacyId} className="border border-slate-100 rounded-lg px-3 py-2">
                      <p className="text-sm font-semibold text-slate-900">{row.sellerName}</p>
                      <div className="flex items-center justify-between text-xs mt-1">
                        <span className="text-slate-500">Income: ₹{row.totalIncome.toLocaleString()}</span>
                        <span className="text-emerald-700 font-semibold">Paid: ₹{row.paidOut.toLocaleString()}</span>
                        <span className="text-amber-700 font-semibold">Pending: ₹{row.pendingPayout.toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
