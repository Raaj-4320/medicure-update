...
const PublicCartPage: React.FC = () => {
  ...
  React.useEffect(() => {
    setWishlistCount(readWishlistIds(wishlistKey, 'PublicCartPage header').length);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <PublicExploreHeader
        ...
        accountReturnTo="/cart"
        searchMode="redirect"
      />
      <div className="max-w-[1180px] mx-auto py-6 px-4 sm:px-6 lg:px-10">
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] xl:grid-cols-[minmax(0,1fr)_360px] gap-5 lg:gap-6">
            <div className="space-y-4">
              {removedItem && (
                <div className="bg-white border border-slate-200 rounded-xl p-3 text-sm flex items-center justify-between shadow-sm">
                  <p className="text-slate-600">Item removed.</p>
                  <button onClick={undoRemove} className="text-emerald-700 font-semibold">Undo</button>
                </div>
              )}
              {cartItems.map((item) => (
                <div key={item.id} className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 flex gap-4 shadow-sm">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl bg-slate-100 overflow-hidden border border-slate-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="font-bold text-slate-900 text-[15px] leading-snug">{item.medicineName || item.brandName || item.name || 'Medicine'}</h3>
                        <p className="text-sm text-slate-500 mt-0.5">₹{Number(item.price || 0).toFixed(2)} each</p>
                      </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                      ...
                      <p className="font-bold text-slate-900 text-base">₹{(Number(item.price || 0) * Number(item.quantity || 0)).toFixed(2)}</p>
                    </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 h-fit lg:sticky lg:top-24 shadow-sm">
