...
export default function OrderDetailsModal({ order, isOpen, onClose, title = 'Order Details' }: OrderDetailsModalProps) {
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-72px)] space-y-6 text-sm">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <p><span className="font-semibold">Customer ID:</span> {renderValue(order.customerId)}</p>
              <p><span className="font-semibold">Seller ID:</span> {renderValue(order.sellerId)}</p>
              <p><span className="font-semibold">Pharmacy ID:</span> {renderValue(order.pharmacyId)}</p>
