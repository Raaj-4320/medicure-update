...
type OrderDetailsModalProps = {
  ...
  medicineNameMap?: Record<string, string>;
};
              ...
              <div className="space-y-2">
                {items.map((item: any, index: number) => (
                  <div key={`${item.medicineId || 'item'}-${index}`} className="border border-slate-200 rounded-lg p-3">
                    <p><span className="font-semibold">Medicine Name:</span> {renderValue(item.brandName || item.name || medicineNameMap[String(item.medicineMasterId || '')] || medicineNameMap[String(item.medicineId || '')] || item.medicineMasterId || item.medicineId)}</p>
                    <p className="text-xs text-slate-500">ID: {renderValue(item.medicineMasterId || item.medicineId)}</p>
